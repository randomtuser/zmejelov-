...
create(){
  ...
  gameState.junak  = this.physics.add.sprite(100,1100, "zmejaDT")
...
var  kordinata =  Math.floor(Math.random() * dolzina+50); 
const napoj =  this.physics.add.sprite(sirina/2, -180 , "napoj")
napoj.setScale(2)
this.physics.add.collider(kolider, napoj)
  ...
  this.physics.add.overlap(gameState.junak, pohlep, () => {
    ...
    if (usa == true){
      this.add.text((dolzina / 2, sirina / 2, gameState.junak.y -150, 'Greed or coincidence?\nAnyway, anything\nthat falls on your head hurts!',{ fontSize: '40px',fill: '#A996BC',  fontFamily: 'CustomFont' }));}
    else if (rus == true){
      this.add.text((dolzina / 2, sirina / 2,  'Жадность или случайность?\nВ любом случае, все,\nчто падает на голову болееть!',{ fontSize: '40px',fill: '#A996BC',  fontFamily: 'CustomFont' }))}
    else{
      this.add.text(dolzina / 2, sirina / 2,  'Pohlep ali naključje?\nKakorkoli, karkoli\nti pade na glavo boli!',{ fontSize: '40px',fill: '#A996BC',  fontFamily: 'CustomFont' });}
