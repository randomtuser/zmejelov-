...
create(){
	...
	this.physics.add.overlap(gameState.junak, portal, () => {
		...
		if (usa == true){
			...
			this.add.text(500, 500,"Уууууууууу похоже на то, что\n ты нашел секретный уровень,\nпоздравляю!!",{ fontSize: '40px',fill: '#A996BC',  fontFamily: 'CustomFont' });}
		else{
			this.add.text(500, 500, "Uuuuuuuuuuuu nekdo je našel\nskrivni nivo,\nčestitkeeee!!",{ fontSize: '40px',fill: '#A996BC',  fontFamily: 'CustomFont' });}
			this.physics.pause();
	...
	this.physics.add.collider(cudo, kolider)
		this.physics.add.overlap(gameState.junak, cudo, () => {
			...
			enkratt = 0
			if (usa == true){
				this.add.text(GAME_WIDTH/2+50, gameState.junak.y +300, "Exploring ruins can be dangerous,\nbut you dared to do it\n😱 ",{ fontSize: '30px',fill: '#A996BC',  fontFamily: 'CustomFont' });}
			else if (rus == true){
				this.add.text(GAME_WIDTH/2+50, gameState.junak.y +300, 'Исследование руин может быть\nопасным,но ты осмелился\n сделать это😱',{ fontSize: '30px',fill: '#A996BC',  fontFamily: 'CustomFont' });}
			else
			{
				this.add.text(GAME_WIDTH/2+50, gameState.junak.y +300, 'Raziskovanje ruševin je\nlahko nevarno,\nti pa si kar upaš😱 ',{ fontSize: '30px',fill: '#A996BC',  fontFamily: 'CustomFont' });}
			}
			});
