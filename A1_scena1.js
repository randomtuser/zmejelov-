...
this.physics.add.overlap(gameState.junak, obvestilo2, () => {
  ...
  if (usa == true){
    this.add.text(450, 250, 'Please note that signs\nlike this will never hurt you', { fontFamily: 'CustomFont', fontSize: 36, color: '#ffffff' });}
  else if (rus == true){
    this.add.text(450, 250,'Обратите внимание, что подобные\nзнаки никогда не повредят вам', { fontFamily: 'CustomFont', fontSize: 36, color: '#ffffff' });}
  else{
    this.add.text(450, 250, 'Zapomni si, da so taki znaki\nedini, ki ti nikoli ne bodo škodili', { fontFamily: 'CustomFont', fontSize: 36, color: '#ffffff' });
    }
  })
